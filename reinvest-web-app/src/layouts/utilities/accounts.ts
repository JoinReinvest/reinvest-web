...
export const getAccountsWithLabel = (availableAccounts: Maybe<AccountOverview>[]): Maybe<AccountsWithAvatarLabel>[] => {
  const individualAccounts = availableAccounts.filter(account => account?.type === AccountType.Individual);
  const corporateAccounts = availableAccounts.filter(account => account?.type === AccountType.Corporate && account?.avatar?.url);
  const trustAccounts = availableAccounts.filter(account => account?.type === AccountType.Trust && account?.avatar?.url);
  const beneficiaryAccounts = availableAccounts.filter(account => account?.type === AccountType.Beneficiary);

  const trustAccountsWithoutAvatar = availableAccounts.filter(account => account?.type === AccountType.Trust && !account?.avatar?.url);
  const corporateAccountsWithoutAvatar = availableAccounts.filter(account => account?.type === AccountType.Corporate && !account?.avatar?.url);
